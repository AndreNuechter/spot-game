...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const plrs = [
    ...
    new Player(slct[1].style.color, 'inactive', Math.pow(boardLen, 2) - boardLen)
  ];
  const gameOver = id('game-over-indicator');
  const circles = [];
  const boardWidth = 7;
  const boardObject = id('board');
  const circleTmpl = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
  let actives;
  let turn;
  ...
  function getEnemyNeighbors(piece) {
    const isPieceNotAtLBorder = piece % boardLen > 0,
      ...
      neighborCells = [
        ...
        isPieceNotAtRBorder ? piece + boardLen + 1 : -1,
        piece + boardLen];
    return neighborCells.filter(cell => 0 <= cell && cell < board.length && board[cell] !== 0 && board[cell] !== actives[turn] + 1);
  }
  ...
  function endTurn() {
    ...
    if (board.filter(cell => cell === 0).length === 0 || plrs.filter(e => e.pieces.length > 0).length <= 1) {
      // find plr placement and communicate end of game
      const places = actives.map(i => [i + 1, plrs[i].pieces.length]).sort((a, b) => b[1] - a[1]);
      places.forEach((pl, i) => {
        ...
        if (circles[o.piece].classList.contains('selected')) {
          ...
          const helper = e => {
            ...
            getEnemyNeighbors(e).forEach(gained => {
              ...
              plr.pieces.push(...owner.pieces.splice(owner.pieces.indexOf(gained), 1));
      ...
      [...o.next, ...o.oneOff].forEach(n => {
        if (most[0] < n.bounty.length + n.type) {
          ...
          if (secMost[0] < n.bounty.length + n.type) {
            ...
            else if (thrdMost[0] === n.bounty.length + n.type) thrdMost[1].push(n);
          } else if (secMost[0] === n.bounty.length + n.type) secMost[1].push(n);
